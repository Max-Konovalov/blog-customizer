...
export const ArticleParamsForm = ({setArticleState}: ArticleParamsProps): JSX.Element => {
	//States
	...
	const [selectedContentWidth, setSelectedContentWidth] = useState(defaultArticleState.contentWidth);
	...
	const handleFontFamilyChange = (fontFamily: typeof defaultArticleState.fontFamilyOption): void => {
		setSelectedFont(fontFamily);
		console.log(`Вы выбрали шрифт: ${fontFamily.title}`);
	};
	const handleFontSizeChange = (size: typeof defaultArticleState.fontSizeOption): void => {
		setSelectedFontSize(size);
		console.log(`Font size is: ${size.title}`);
	};
	const handleFontColorChange = (color: typeof defaultArticleState.fontColor): void => {
		setSelectedFontColor(color);
		console.log(`Font color is: ${color.title}`);
	};
	const handleBackgroundColorChange = (bgColor: typeof defaultArticleState.backgroundColor): void => {
		setSelectedBackgroundColor(bgColor);
		console.log(`Background color is: ${bgColor.title}`);
	};
	const handleContentWidthChange = (width: typeof defaultArticleState.contentWidth): void => {
